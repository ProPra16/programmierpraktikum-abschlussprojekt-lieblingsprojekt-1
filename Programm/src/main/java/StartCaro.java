...
public class StartCaro extends Application {
	...
	private Parent createContent() {
		...
		red = new Button("RED");
		red.setStyle("-fx-background-color: LIGHTPINK;"); // kann man machen,
															// wenn man will
		...
		green.setStyle("-fx-background-color: LIGHTGREEN;"); // kann auch wieder
																// raus
		...
		aktzeptanzCheckbox.setOnAction(new EventHandler<ActionEvent>() {
			public void handle(ActionEvent ae) {
				...
				testErfolgreich = compiliere(gesammteTestCode, klasseTest.getName(), klasseMain.getCode(),
						klasseMain.getName(), textKonsole);
				if (testErfolgreich) {
					textKonsole.setText("Das Programm ist nicht in Ordnung, \nbitte Main weiter abaendern");
					...
					textKonsole.setText("Das Programm ist in Ordnung.");
				}
		...
		// ab hier kümmern: klasse test und main + akzeptanztest kompilieren,
		// dann schreibe neue wenn der richtug ist
		pruefeProg.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent ae) {
				...
				if (testErfolgreich) {
					...
					textKonsole.setText(
							"Erfolgreich! \nDu kannst die Test Methoden und \ndas Main Programm anpassen, \nsowie den AkzeptanzTest Checken.");
	...
	public Parent akzept_test(Stage stage_akzeptanz) throws IOException {
		...
		checker.setOnAction(new EventHandler<ActionEvent>() {
			@Override
			public void handle(ActionEvent ae) {
				geladen = true; // reingebracht damit es wieder klappt
				stage_akzeptanz.close();
				System.out.println("1");
				/*
				 * if(compiliere(akzept_text.getText(), klasseTest.getName(),
				 * textProgrammInhalt, klasseMain.getName(), textKonsole)){
				 * geladen=true; textKonsole.setText(
				 * "AkzeptanzTest noch nicht erfuellt, Programm muss bearbeitet werden."
				 * );
				 * 
				 * } else{ textKonsole.setText("Gesammte Programm in Ordnung.");
				 * }
				 */ // auskommentiert weil das nicht funktioniert
