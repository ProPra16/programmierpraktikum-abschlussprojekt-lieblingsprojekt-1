...
public class StartCaro extends Application {
	...
	//ab hier kümmern: klasse test und main + akzeptanztest kompilieren, dann schreibe neue wenn der richtug ist 	
    	...
    	try{
    		...
    		if(result){
    			...
    			if(javaCompilers.getTestResult().getNumberOfFailedTests() >= 1){    				
    				...
    				textKonsole.setText(konsolentext+ "\n" + "Error failed Tests: " + javaCompilers.getTestResult().getNumberOfFailedTests()+" "+ sTest +" "+ sMain);      			
    				return true;
    			}
    			else{
    			return false;  	
